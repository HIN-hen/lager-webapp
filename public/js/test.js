...
// add back to canvas from history.
function redo() {
...
// remove from canvas out of history.
function undo() {
  if (i <= 0) return reset();
  i--;
  ctx.putImageData(history[i],0,0);
...
// Test images for drawing on image.
const image = new Image();
image.src = "../images/test-361512_640.webp";
let pat = null;
image.onload = () => {
  pat = ctx.createPattern(image, 'no-repeat');
}

// init drawing on canvas
function init() {
  baseLine();
  // Test code mit fill style.
  canvas.width = 640;
  canvas.height = 640;
  ctx.fillStyle = pat;
  ctx.fillRect(0, 0, canvas.width, canvas.height);
